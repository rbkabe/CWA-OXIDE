...
use crate::game_server::{
    handlers::{
        ...
        chat_command::{HOLOPROJECTOR_MODELS, MIND_TRICKS, PERSISTENT_MIND_TRICKS, WEAPON_MOVES, weapon_move_animation_id},
            ...
            game_server
                ...
                .read_characters(|_| CharacterLockRequest {
                    ...
                    character_consumer: move |ctable, _, mut cwrite, _| {
                        ...
                        // ── Weapon Move ──────────────────────────────────────────────────
                        // Weapon move quick_chats have animation_id=0; the clip
                        // depends on the player's equipped weapon class.  Resolve via
                        // WEAPON_MOVES + weapon_move_animation_id, mirroring
                        // RequestStartCast in ability.rs.
                        if let Some((_, _, pack, move_index)) =
                            WEAPON_MOVES.iter().find(|(_, guid, ..)| *guid == item_guid)
                        {
                            let pack = *pack;
                            ...
                            if let CharacterType::Player(p) = &ch.stats.character_type {
                                if let Some(animation_id) =
                                    weapon_move_animation_id(p, game_server, pack, move_index)
                                {
