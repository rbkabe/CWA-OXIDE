...
impl GameServer {
    ...
    pub fn process_packet(
        ...
        crate::debug!(
            "process_packet from {sender}: raw_op_code={raw_op_code} (0x{raw_op_code:x}), decoded={:?}",
            OpCode::try_from(raw_op_code).ok()
        );
        ...
        match OpCode::try_from(raw_op_code) {
            Ok(op_code) => match op_code {
                                ...
                                zones_lock_enforcer.read_zones(|_| ZoneLockRequest {
                                    ...
                                    zone_consumer: |_, zones_read, _| {
                                        ...
                                        if let CharacterType::Player(player) = &character_write_handle.stats.character_type {
                                            ...
                                            // Send the player's full equipped-item set (gear +
                                            // sabers) via EquipItem/UnequipItem now that the
                                            // client has signaled it's actually ready -- sending
                                            // these during log_in() crashes the client because
                                            // the UI/equipment cache isn't initialized yet.
                                            // update_player_equipped_items is a superset of the
                                            // old update_saber_tints (same tint-override logic
                                            // for saber shape/color slots), so it fully replaces
                                            // that call. Its trailing UpdateWieldType broadcast
                                            // is dropped since wield type is already broadcast
                                            // below using the authoritative, brandish-aware value.
                                            let other_players_nearby_for_equip = ZoneInstance::other_players_nearby(
                                                Some(sender),
                                                chunk,
                                                instance_guid,
                                                characters_table_read_handle,
                                            );
                                            let (mut equip_broadcasts, _wield_type) = update_player_equipped_items(
                                                sender,
                                                &player.inventory,
                                                other_players_nearby_for_equip,
                                                self,
                                            );
                                            crate::debug!(
                                                "ClientIsReady from {sender}: equipped_items={:?}, equip_broadcast_packet_counts={:?}",
                                                player.inventory.equipped_items(player.inventory.active_battle_class),
                                                equip_broadcasts.iter().map(|b| match b {
                                                    Broadcast::Single(_, packets) => packets.len(),
                                                    Broadcast::Multi(_, packets) => packets.len(),
                                                }).collect::<Vec<_>>()
                                            );
                ...
                OpCode::Store => {
                    broadcasts.append(&mut process_store_packet(&mut cursor, sender, self)?);
                }
                OpCode::Housing => {
                ...
                OpCode::Purchase => {
                    ...
                    // StoreCategories/Billboards/StoreCategoryGroups in
                    // purchase.rs). No struct exists yet for whatever the
                    // client sends here on its own - logging raw bytes to
                    // capture what a Consumable action-bar slot click
                    // actually sends (see test_data.rs slot 5 NOTE / task
                    // #40).
